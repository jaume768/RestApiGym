...
export const updateEntreno = async (req,res) =>{ 
    const {id} = req.params
    const {Kilos,Series,Repes,Pr,Fecha} = req.body
    ...
    console.log(result)
    const [historial] = await pool.query('insert into horario(Kilos,Repes,Series,Pr,Fecha,id_entrenamiento) values(?,?,?,?,?,?)',[Kilos,Repes,Series,Pr,Fecha,id])
