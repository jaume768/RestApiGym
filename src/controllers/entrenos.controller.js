...
export const updateEntreno = async (req,res) =>{ 
    const {id} = req.params
    const {Kilos,Series,Repes,Pr,Dia,Mes,Anyo} = req.body
    ...
    console.log(result)
    const [historial] = await pool.query('insert into horario(Kilos,Repes,Series,Pr,Dia,Mes,Anyo,id_entrenamiento) values(?,?,?,?,?,?,?,?)',[Kilos,Repes,Series,Pr,Dia,Mes,Anyo,id])
...
export const getHistorial = async (req,res) =>{ 
    const {id} = req.params
    const [result] = await pool.query('Select entrenamientos.Nombre,horario.Kilos,horario.Repes,horario.Series,horario.Pr,horario.Dia,horario.Mes,horario.Anyo from horario,entrenamientos where entrenamientos.id = horario.id_entrenamiento and entrenamientos.id = ? ORDER BY "horario"."Mes" ASC',id)
    res.json(result)
}