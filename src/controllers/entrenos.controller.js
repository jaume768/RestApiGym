...
export const getHistorial = async (req,res) =>{ 
    const {id} = req.params
    const [result] = await pool.query('Select entrenamientos.Nombre,horario.Kilos,horario.Repes,horario.Series,horario.Pr,horario.Dia,horario.Mes, from horario,entrenamientos where entrenamientos.id = horario.id_entrenamiento and id = ?',id)
    res.json(result)
}